...
const toTitle = (s) =>
  (s || "")
    ...
    .replace(/[_-]+/g, " ")
    .replace(/\s+/g, " ")
    // capitalizar primeras letras (incluye acentos)
    .replace(/\b\p{L}/gu, (m) => m.toUpperCase())
    .trim();

const DEP_MAP = new Map(
  DEPARTAMENTOS.map((o) => [depKey(o.value || o.label), o.value || o.label])
);
...
const parseEstadoExcel = (v) => {
  ...
  if (
    ...
    return true;
  if (
    ["0", "false", "no", "no habilitado", "deshabilitado", "inactivo"].includes(
      s
    )
  )
    return false;
...
export default function Adherente() {
  ...
  // Filtro Estado
  const [estadoSeleccionado, setEstadoSeleccionado] = useState(null); // UI: true/false/null
  const [estadoAplicado, setEstadoAplicado] = useState(null); // aplicado
  ...
  const [form, setForm] = useState({
    ...
    estado: true,
    observaciones: "",
    adherente: true, // en esta página, siempre true
  });
  ...
  useEffect(() => {
    ...
    const qRef = deptoAplicado
      ? query(base, where("departamento", "==", deptoAplicado))
      : base;
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        ...
        toast.current?.show({
          severity: "error",
          summary: "Error",
          detail: "No se pudo leer la información.",
        });
        setLoading(false);
      }
    );
  ...
  // ===== Búsqueda + filtro Estado en cliente =====
  const dataFiltrada = useMemo(() => {
    const s = normalizeText(busqueda).trim();
    let base = [...rows];

    if (estadoAplicado !== null)
      base = base.filter((r) => !!r.estado === estadoAplicado);
    ...
    return base.filter((r) => {
      ...
      const txt = normalizeText(
        `${r.apellido || ""} ${r.nombre || ""} ${r.dni || ""} ${
          r.departamento || ""
        } ${establecimiento} ${r.celular || ""} ${r.email || ""} ${estadoTxt} ${
          r.observaciones || ""
        } ${r.tituloGrado || ""} ${r.descuento || ""} ${r.nroAfiliacion || ""}`
      );
      return txt.includes(s);
    });
  }, [rows, busqueda, estadoAplicado]);

  // ===== Exportar Excel (usa la lista filtrada) =====
  ...
  const importExcel = async (file) => {
    ...
    const rows = XLSX.utils.sheet_to_json(ws, { header: 1 }); // matriz [filas][columnas]
    ...
    for (const arr of rows.slice(1)) {
      ...
      const payload = {
        ...
        establecimiento: String(getVal("establecimientos") || "").trim(), // compat
      ...
      // Upsert por DNI
      const qRef = query(
        collection(db, ADHERENTES_COLLECTION),
    ...
    toast.current?.show({
      ...
      detail: `Insertados: ${inserted} • Actualizados: ${updated} • Omitidos: ${skipped}`,
      life: 5000,
    });
  };
  ...
  const abrirEditar = (row) => {
    ...
    setForm({
      ...
      descuento:
        typeof row.descuento === "string" ? row.descuento : row.descuento ?? "",
  ...
  const guardar = async () => {
    ...
    try {
      const payload = {
        ...
        dni: form.dni.trim(),
        nroAfiliacion:
          form.nroAfiliacion !== "" ? Number(form.nroAfiliacion) : null,
        tituloGrado: form.tituloGrado?.trim() || "",
        descuento: normalizeDescuento(form.descuento), // "si" | "no" | ""
        departamento: toCanonicalDepartamento(form.departamento || ""),
        establecimientos: form.establecimientos?.trim() || "",
        establecimiento: form.establecimientos?.trim() || "", // compat con registros viejos
  ...
  const toggleEstado = async (row) => {
    try {
      await updateDoc(doc(db, ADHERENTES_COLLECTION, row.id), {
        estado: !row.estado,
      });
    } catch (err) {
  ...
  const runCotizante = async () => {
    ...
    if (!dniKey || nro == null) {
      toast.current?.show({
        ...
        detail:
          "Faltan DNI o N° de afiliación para actualizar el registro en “nuevoAfiliado”.",
      });
      return;
    }
    ...
    setCotzSaving(true);
    try {
      // 1) Buscar en "nuevoAfiliado" por dni + nroAfiliacion
      //    Intentamos como número y como string (por heterogeneidad de tipos)
      const qNum = query(
      ...
      if (targets.length === 0) {
        toast.current?.show({
          ...
          detail:
            "No se encontró el afiliado en “nuevoAfiliado” con ese DNI y N° de afiliación.",
          life: 5000,
        });
        setCotzSaving(false);
        return;
      ...
      // 2) Actualizar "observaciones" y marcar adherente = false en nuevoAfiliado
      const trimmedObs = (cotzObs ?? "").trim();
      await Promise.all(
        targets.map((dref) =>
          updateDoc(dref.ref, {
            observaciones: trimmedObs,
            adherente: false, // ✅ pasa a NO (booleano)
          })
        )
      );
      ...
      toast.current?.show({
        ...
        detail:
          "Se actualizó “observaciones” en nuevoAfiliado y se eliminó el registro en Adherentes.",
      });
      ...
      toast.current?.show({
        severity: "error",
        summary: "Error",
        detail: "No se pudo completar la operación Cotizante.",
      });
    } finally {
      setCotzSaving(false);
    }
  ...
  const confirmarBorrado = (row) => {
    confirmDialog({
      header: "Confirmar",
      message: `¿Eliminar a ${row.apellido ?? ""} ${
        row.nombre ?? ""
  ...
  const estadoBodyTemplate = (row) => (
    <Tag
      value={row.estado ? "Habilitado" : "No habilitado"}
  ...
  const leftToolbar = (
    <div className={styles.toolbarLeft}>
      ...
      <Button
        ...
        disabled={loading || dataFiltrada.length === 0}
      />
      <input
        ref={fileInputRef}
        type="file"
    ...
    <div className={styles.toolbarRight}>
      ...
      <Button
        ...
        onClick={() => {
          ...
          setEstadoAplicado(
            typeof estadoSeleccionado === "boolean" ? estadoSeleccionado : null
          );
          setFirst(0);
        }}
      />
    ...
    <div className={styles.container}>
      ...
      <div className={styles.header}>
        ...
        <p className={styles.subtitle}>
          En esta sección podés gestionar afiliados adherentes: crear registros,
          importar o descargar Excel, buscar y filtrar por Departamento y
          Estado; además ver y editar, habilitar/deshabilitar o eliminar, y
          consultar su situación de pago o deuda (si corresponde).
        </p>
      </div>
      ...
      >
        ...
        <Column
          field="departamento"
          header="Departamento"
          body={(r) => toCanonicalDepartamento(r.departamento)}
          sortable
        />
        <Column
          field="establecimientos"
          header="Establecimiento"
          body={(r) => r.establecimientos || r.establecimiento || "—"}
        />
        ...
        <Column
          ...
          className={styles.observacionesCol}
        />
      ...
      >
        <div className={styles.viewGrid}>
          <div>
            ...
            {rowVer
              ? `${rowVer.apellido || "—"}, ${rowVer?.nombre || "—"}`
              : "—"}
          </div>
          <div>
            <strong>DNI:</strong> {rowVer?.dni || "—"}
          </div>
          <div>
            <strong>Afiliación:</strong>{" "}
            {afiliacionLabel(rowVer?.nroAfiliacion)}
          </div>
          <div>
            <strong>Título de grado:</strong> {rowVer?.tituloGrado || "—"}
          </div>
          <div>
            <strong>Descuento:</strong> {toSiNo(rowVer?.descuento)}
          </div>
          <div>
            <strong>Departamento:</strong>{" "}
            {toCanonicalDepartamento(rowVer?.departamento) || "—"}
          </div>
          <div>
            <strong>Establecimiento:</strong>{" "}
            {rowVer?.establecimientos || rowVer?.establecimiento || "—"}
          </div>
          <div>
            <strong>Celular:</strong> {rowVer?.celular || "—"}
          </div>
          <div>
            <strong>Email:</strong> {rowVer?.email || "—"}
          </div>
          ...
          <div>
            <strong>Adherente:</strong> Sí
          </div>
          <div>
            <strong>Estado:</strong>{" "}
            <Tag
              value={rowVer?.estado ? "Habilitado" : "No habilitado"}
              severity={rowVer?.estado ? "success" : "danger"}
            />
          </div>
        </div>
      </Dialog>
        ...
        <div className={styles.formGrid}>
          <div className={styles.formRow}>
            ...
            <InputText
              value={
                cotzRow
                  ? `${cotzRow.apellido || ""}, ${cotzRow.nombre || ""}`
                  : ""
              }
              readOnly
            />
          ...
          <div className={styles.formRowFull}>
            <label>Observaciones (se guardará en “nuevoAfiliado”)</label>
            <InputTextarea
              autoResize
              rows={3}
              value={cotzObs}
      ...
      >
        <div className={styles.center}>
          ...
          <p style={{ marginTop: 12 }}>
            Procesando registros… Esto puede tardar según el tamaño del archivo.
          </p>
        </div>
      </Dialog>
    </div>
